...
class InstagramSpider(scrapy.Spider):
    ...
    def get_api_follow(self, response, user_data):
        if b'application/json' in response.headers['Content-Type']:
            data = response.json()
            yield from self.get_follow_item(user_data, data['data']['user']['edge_follow']['edges'], None)
    ...
    def get_api_followers(self, response, user_data):
        if b'application/json' in response.headers['Content-Type']:
            data = response.json()
            yield from self.get_follow_item(user_data, None, data['data']['user']['edge_followed_by']['edges'])
    ...
    def get_follow_item(self, user_data, follow_users_data, follower_users_data):
